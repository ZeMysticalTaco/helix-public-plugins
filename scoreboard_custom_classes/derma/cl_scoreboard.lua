...
function PANEL:SetModel( model, skin, bodygroups )
    ...
    self.model = model
    self.skin = skin
    self.path = "materials/spawnicons/" .. model:sub( 1, #model - 4 ) .. ( ( isnumber( skin ) and skin > 0 ) and ( "_skin" .. tostring( skin ) ) or "" ) .. ( self.bodygroups ~= BODYGROUPS_EMPTY and ( "_" .. self.bodygroups ) or "" ) .. ".png" -- remove extension -- skin number -- bodygroups
...
function PANEL:Update( )
    ...
    if ( localCharacter and character ) then
        bRecognize = hook.Run( "IsCharacterRecognized", localCharacter, character:GetID( ) ) or hook.Run( "IsPlayerRecognized", self.player )
    end

    self.icon:SetHidden( not bRecognize )
...
function PANEL:Think( )
    if ( CurTime( ) >= self.nextThink ) then
        ...
        if ( not IsValid( client ) or not client:GetCharacter( ) or self.character ~= client:GetCharacter( ) or self.team ~= client:Team( ) ) then
            self:Remove( )
...
function PANEL:AddPlayer( client, index )
    if ( not IsValid( client ) or not client:GetCharacter( ) or hook.Run( "ShouldShowPlayerOnScoreboard", client ) == false ) then return false end
...
function PANEL:SetFaction( faction )
    self:SetColor( ix.config.Get('color') ) --TODO match player color brother.
    self:SetText( faction )
...
function PANEL:Update( )
    ...
    for k, v in pairs( player.GetAll( ) ) do
        if v:GetNetVar( 'customClass', false ) then
            hasPlayers[ v:GetNetVar( 'customClass' ) ] = true
        else
            hasPlayers[ ix.faction.indices[ v:Team( ) ].name ] = true
        end
    end

    for k, v in pairs( ix.gui.scoreboard.factions ) do
        if not hasPlayers[ k ] then
            ...
            for k2, v2 in pairs( player.GetAll( ) ) do
                if not v2:GetCharacter( ) then continue end --TODO: make 'unloaded character scoreboard slot'

                if ( not IsValid( v2.ixScoreboardSlot ) ) then
                    local class = (v2:GetNetVar('customClass', false) and v2:GetNetVar('customClass') or ix.faction.indices[v2:Team()].name)
                    if self.faction ~= class then continue end
                    if ( self:AddPlayer( v2, k2 ) ) then
                        bHasPlayers = true
                    end
                else
                    print(v2.ixScoreboardSlot)
                    v2.ixScoreboardSlot:Update( )
                    bHasPlayers = true
                end
            end
...
function PANEL:Init( )
    ...
    for k, v in pairs( player.GetAll( ) ) do
        ...
        if not v:GetNetVar( 'customClass', false ) then
            identifier = ix.faction.indices[ v:Team( ) ].name
        else
            identifier = v:GetNetVar( 'customClass' )
        end
        --I think this might re-initialize the class panel each time.
        if not self.factions[identifier] then
        local panel = self:Add( "ixScoreboardFaction" )
        panel:SetFaction( identifier )
        panel:Dock( TOP )
        self.factions[ identifier ] = panel
        end
        --PrintTable( self.factions )
    end
...
function PANEL:Think( )
    if ( CurTime( ) >= self.nextThink ) then
        ...
        for k, v in pairs( self.factions ) do
            --local factionPanel = self.factions[identifier]
            v:Update( )
        end
